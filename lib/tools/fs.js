...
exports.execMultiple = function (commands, options, cb) {
  ...
  var next = function () {
    ...
    if (_cmd.cwd)
      options.cwd = _cmd.cwd;
    if (_cmd.timeout)
      options.timeout = _cmd.timeout;
    ...
    //console.log('execMultiple', _cmd.command, options);
    cp.exec(_cmd.command, options, function (error, stdout, stderr) {
      if (!error) {
        ...
        ret.errors = true;
        if (!_cmd.skipError && !options.skipError) {
          // exit on first error
